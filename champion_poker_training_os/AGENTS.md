...
| **Math Lab doğruluk** | `app/db/seed_data.py` `generate_math_drills()` (150 drill, 9 kategori). Formüller DOĞRULANDI + regresyon testi (`tests/test_math_lab.py`): pot-odds = call/(pot+2·call), alpha = bet/(bet+pot), MDF = pot/(pot+bet) = 1−alpha, EV = w·reward−(1−w)·risk, ICM = chipEV+risk_premium, combos (C(4,2)=6, deck=1326…). Test cevapları bağımsız reimplementasyonla eşleştirir |
