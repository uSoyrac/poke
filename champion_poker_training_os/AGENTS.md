...
| **Live GTO on buttons** | `app/poker/gto_live_advice.py` — `live_gto_advice(hand, hero_idx, mode)` maps live state → GTO scenario (RFI/vs-RFI/vs-3bet/push-fold via position + betting history + stack) → action frequencies. play_session + tournament `_update_action_buttons` append `42%` to each button. **Preflop** = curated/heuristic (EXACT/APPROX). **Postflop** = `_postflop_advice` equity-based model (🟠 CONCEPT): MC equity vs an action-aware villain continuing range (`_villain_continuing_range(n, bet_frac)` tightens vs bigger bets), then pot-odds→frequency mapping. `LiveAdvice.equity` field carries hero equity %. Honest: postflop direction is right but not solver-exact. Also writes `AppState.live_gto` (incl. pot_bb/to_call_bb/equity/sizing) for the coach + reveal panel |
