...
| **Postflop GTO beyni** | `app/poker/postflop_gto.py` (Phase D3, saf fonksiyon) — `classify_board(community) → BoardTexture` (paired/monotone/two_tone/connected/wetness/label). `cbet_strategy(eq, tex, in_position, has_initiative, street, n_active) → (bet_freq, size)`: kuru board → yüksek c-bet küçük size (range bet), ıslak → düşük/polarize büyük size, OOP/inisiyatifsiz → daha çok check. **D3+**: sokak barrel azalması (turn ×0.85, river ×0.72; size büyür) + multiway sıkılaşma (3+ oyuncu ×0.7/0.5). `defend_strategy(eq, tex, pot, to_call, n_active) → (fold,call,raise)`: MDF + equity + ıslakta semi-bluff raise; multiway daha sıkı (fold↑, bluff-raise↓). Solver-exact değil, GTO-ŞEKLİ doğru. Defaults (flop, 2 oyuncu) D3 davranışını korur. **D6 spot-tipleri**: `BoardTexture.caller_favored` (≤9-high, unpaired → savunan range avantajı); donk-lead board'a bağlı (caller-favored ×0.45, A/K/Q ×0.12 ≈ yok); `probe=True` (agresör önceki sokağı check-back yaptı → `_prev_street_checked_through`) probe bet ×0.65 |
...
| **Hero-decision persistence + Leak Finder** | `app/db/repository.py` — `record_decision_log(log)` writes each hand's `_decision_log` snapshots to `hero_decisions` (cols added via `_ensure_decision_columns` migration: `category`, `street`, `created_at`). Called in play_session `_on_complete`/`_on_complete_mtt`. `frequency_error = 100 − (GTO% of hero's chosen action)`; `ev_loss` from equity+pot-odds (+EV fold or −EV call). `get_decision_leaks(min_sample=8)` buckets by category (RFI/vs-RFI/vs-3bet/Postflop/Push-Fold) → over-fold / spew / deviation leaks. `get_leak_analysis()` merges played-hands stats (VPIP/WTSD/W$SD) + decision leaks. **`app/ui/screens/leak_finder.py`** `LeakFinderScreen` is now **data-driven**: `reload()` pulls `get_leak_analysis()`, banner shows real-vs-example state, static `EXTENDED_LEAKS` only as fallback when data is thin. `main.navigate()` calls `reload()` on any screen that has it. **Phase D4**: "🔧 Hatalarımdan Drill Üret" → `DrillLibrary.generate_repair_pack(leaks)` gerçek leak'lerden ağırlıklı tamir paketi (Critical=6/High=4/Medium=2 drill — ağır leak → daha çok tekrar = spaced-repetition), Spot Practice Trainer'a yönlendirir. `generate_from_leak(leak, count)` artık parametrik. **GTO ilerleme**: `get_gto_accuracy_trend(days)` hero_decisions'ı güne göre gruplar → günlük doğruluk % (freq_error≤65 ≈ A/B) + ort. EV kaybı; Leak Finder üstünde sparkline şeridi ("GTO İLERLEME ▄▅▆▇█ %40→%67 ↑") |
| **Gerçek el geçmişi import** | `app/poker/hand_history_import.py` (Phase D7, saf fonksiyon) — `parse_pokerstars(text) → list[played_hand dict]`. PokerStars NLHE cash formatı: hero "Dealt to" satırından, net kâr = collected + uncalled_returned − invested (street-commit takibi; blind'ler preflop'a dahil, FLOP/TURN/RIVER'da commit sıfırlanır), bb-normalize. `repository.import_hands_from_text(text)` parse + `save_played_hand`. Hand History Archive ekranında "📥 PokerStars HH Import" butonu (QFileDialog). Gerçek online eller → istatistik/leak/GTO-ilerleme analizine katılır |
| **Reports — GTO gelişim** | `app/ui/screens/reports.py` (Phase D8) — üstte "GTO GELİŞİM · gerçek oyun verisi" bölümü: genel doğruluk, 14-gün delta (↑/↓/→), kategori kırılımı (en zayıf önce), gün-gün trend MiniChart. `reload()` `get_gto_accuracy_trend` + `get_gto_category_accuracy`'den besler; veri yoksa graceful boş durum. `main.navigate()` reload tetikler |
| **Math Lab doğruluk** | `app/db/seed_data.py` `generate_math_drills()` (150 drill, 9 kategori). Formüller DOĞRULANDI + regresyon testi (`tests/test_math_lab.py`): pot-odds = call/(pot+2·call), alpha = bet/(bet+pot), MDF = pot/(pot+bet) = 1−alpha, EV = w·reward−(1−w)·risk, ICM = chipEV+risk_premium, combos (C(4,2)=6, deck=1326…). Test cevapları bağımsız reimplementasyonla eşleştirir. **Push/fold drill'leri GERÇEK MC equity** (`_pushfold_equity_drills`, cache+seed): hero combo vs top-% range — elle-tahmin değil |
| **Bot fidelity (tam geçer)** | `tests/test_bot_archetype_fidelity.py` artık deterministik (seed) + POOLED oran + örneklem-kapısı (FCB≥40, AF≥25 call) — sıkı botların ölçülemez düşük-örneklem metrikleri yanlış-alarm vermez. `bot_brain` c-bet cap'i GTO-gerçekçi (~%50-65) düşürüldü → AF dengeli. **0 archetype sapması** (D23) |
| **Hatalarımı Tekrar Oyna** | `repository.mistake_spots` tablosu + `get_mistake_spots()` — GTO'dan ciddi sapınca (freq_err≥60 veya ev_loss>1) GERÇEK spot (board/kart/pozisyon/pot) kaydedilir; Spot Practice Trainer "🔁 Hatalarım" kaynağı bu spotları masada yeniden oynatır (D20). `decision_capture` snapshot'ı bunun için hero_position/hero_cards_disp/n_active/raiser_pos taşır |
| **Oyun-içi sızıntı = YOK** | Aksiyon butonları oyun sırasında ASLA GTO %/cevabı göstermez (D14, her iki mod). GTO sadece el sonu reveal'de. Reveal **durum-bazlı**: "👤 BB · A♦K♥ · 6 oyuncu · SRP · CO açtı" (D15). Sizing karşılaştırması "sen 5bb → GTO ~12bb KÜÇÜK" (D10). Turnuvada el oynadıysan SPACE bekler — panel kaçmaz (D14). Graded reveal'de EXACT solver arka planda otomatik (D19) |
| **Trainer masa gerçekçiliği** | `spot_table.parse_cards` bitişik ("AsKh") + boşluklu formatı destekler → ICM/Math masasında hero eli açık görünür (D16). `_table_size` 2-9 parse eder; ICM drill `min(players_left,9)` koltuk → 9-handed bubble 9 koltuk çizer (D17). ICM Trainer decide→review→next döngüsü (spot kaçmaz, GTO-doğru vurgulanır) (D18) |
| **On-demand EXACT solver** | `app/poker/postflop_solver.py` — `solve_spot_exact(board, pot, stack, IP, combo, pot_type)` TexasSolver'ı arms-length çağırır (binary repo'da DEĞİL; AGPL). **Pot-tipine duyarlı range'ler** (`_POT_TYPE_RANGES`: SRP/3BP/4BP — 3-bet/4-bet potta iki taraf sıkı). `decision_capture.preflop_pot_type(hand)` preflop raise sayısından pot tipini bulur, snapshot'a yazılır. Reveal panelindeki "🎯 EXACT" butonu (QThread, `_SolveWorker`) bu veriyle çözer; CONCEPT → gerçek solver frekansı |
